...
ENV_EXAMPLE="$ROOT_DIR/deploy/production/.env.production.example"
...
if [[ $# -ne 0 ]]; then
  usage >&2
  die "This prod script does not accept arguments."
fi
...
if [[ ! -f "$ENV_FILE" ]]; then
  if [[ -f "$ENV_EXAMPLE" ]]; then
    ...
    echo "Created production env file from example: $ENV_FILE"
  fi
  die "Edit $ENV_FILE with production domain/IP and passwords, then run deploy/prod.sh again."
fi
